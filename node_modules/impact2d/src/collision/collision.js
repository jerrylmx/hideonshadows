...
export default class Collision {
  ...
  static timeTravel(body1, body2, refreshFn) {
    ...
    let hitPoints = [];
    if (body2.type === 'C') {
      hitPoints = Util.circleLineSegIntersect(body2.x, body2.y, body2.r, body1.lastPos, {x: body1.x, y: body1.y})
    } else {
      let faces = Util.getFaces(body2.getVerticesWorld());
      faces.forEach((pair) => {
        let res = Util.lineSegLineSegIntersect(body1.lastPos, {x: body1.x, y: body1.y}, pair[0], pair[1]);
        hitPoints = [...hitPoints, ...res];
      });
    }
    if (hitPoints.length === 0) return;
    let closestDist = Util.distSq(body1, hitPoints[0]);
    let contact = hitPoints[0];
    hitPoints.forEach((p) => {
      let dist = Util.distSq(body1, p);
      if (dist < closestDist) {
        closestDist = dist;
        contact = p;
      }
