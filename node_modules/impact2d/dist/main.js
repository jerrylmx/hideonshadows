...
class Collision {
  static handle(body1, body2, delta, refreshFn) {
    // Fast moving object handling
    if (body1.percise) this.timeTravel(body1, body2, refreshFn);
    if (body2.percise) this.timeTravel(body2, body1, refreshFn);
    ...
    if (body1.type === 'C' && body2.type === 'C') {
      return Collision.handleCC(body1, body2, delta, false);
    } else if (body1.type === 'C' && body2.type === 'P') {
      return Collision.handleCP(body1, body2, delta, false);
    } else if (body1.type === 'P' && body2.type === 'C') {
      return Collision.handleCP(body2, body1, delta, false);
    } else {
      return Collision.handlePP(body2, body1, delta, false);
    }
  }
  ...
  static timeTravel(body1, body2, refreshFn) {
    if (!body1.lastPos || body2.percise) return;
    let hitPoints = [];
    if (body2.type === 'C') {
      hitPoints = _libs_util__WEBPACK_IMPORTED_MODULE_0__["default"].circleLineSegIntersect(body2.x, body2.y, body2.r, body1.lastPos, {x: body1.x, y: body1.y})
    } else {
      let faces = _libs_util__WEBPACK_IMPORTED_MODULE_0__["default"].getFaces(body2.getVerticesWorld());
      faces.forEach((pair) => {
        let res = _libs_util__WEBPACK_IMPORTED_MODULE_0__["default"].lineSegLineSegIntersect(body1.lastPos, {x: body1.x, y: body1.y}, pair[0], pair[1]);
        hitPoints = [...hitPoints, ...res];
      });
    }
    if (hitPoints.length === 0) return;
    let closestDist = _libs_util__WEBPACK_IMPORTED_MODULE_0__["default"].distSq(body1, hitPoints[0]);
    let contact = hitPoints[0];
    hitPoints.forEach((p) => {
      let dist = _libs_util__WEBPACK_IMPORTED_MODULE_0__["default"].distSq(body1, p);
      if (dist < closestDist) {
        closestDist = dist;
        contact = p;
      }
...
class Engine {
  ...
  /**
   * Safely remove a body during simulation
   */
  removeEntityRuntime(e) {
    if (e.eternal) return;
    e.dead = true;
  }

  /**
   * Remove a body (Do not use externally!)
   */
  removeEntity(e) {
    if (e.eternal) return;
    try {
      ...
      console.log('Unsuccessful pop operation, doing brute pop now...');
      this.grid.popBrute(e);
    } finally {
  ...
  refreshGrid(e) {
    ...
    if (e.x > this.cfg.scale || e.x < 0 || e.y > this.cfg.scale || e.y < 0) {
      console.log(`Found out of bound entity ${e.id}`);
      e.dead = true;
      return false;
    }
    try {
  ...
  tick(delta = this.delta) {
    ...
    for (const id in this.entities) {
      ...
      // for (const id2 in this.entities) {
        ...
        let n = neighbours[i];
        if (n.id !== e.id) {
          if (e.ignoreIf && e.ignoreIf(n)) continue;
          if (n.ignoreIf && n.ignoreIf(e)) continue;
          if (e.dead || n.dead) continue;
...
class Grid {
  ...
  popBrute(entity) {
    if (this.isLeaf()) {
      ...
      this.items.forEach(child => child.popBrute(entity));
    }
  }

  pop(entity) {
    // Supposed to find the entity in the calculated leaf node
    if (this.isLeaf()) {
      let a = this.payload.length;
      this.payload = this.payload.filter(e => e.id !== entity.id);
      let b = this.payload.length;
      if (a === b) {
        console.log('Unable to locate entity during pop');
      }
      return;
    }
    if (!this.isIn(entity)) {
...
class Entity {
  constructor(cfg) {
    ...
    // Fast moving object mode
    this.percise = cfg.percise;
    this.lastPos = null;
